...
| WebhookHandlerFunction | `AWS::Lambda::Function` | Node.js 20, ARM64, 256 MB, 30s timeout. Validates HMAC signature and enqueues events. |
| ReviewAgentFunction | `AWS::Lambda::Function` | Node.js 20, ARM64, 1024 MB, 300s timeout. Runs the multi-agent review pipeline. |
| ReviewQueue.fifo | `AWS::SQS::Queue` | FIFO queue with content-based deduplication. Visibility timeout of 360s. |
...
<AccordionGroup>
  ...
  <Accordion title="SQS">
    ...
    Scoped to the ReviewQueue FIFO queue. WebhookHandler sends messages; ReviewAgent receives and deletes them.
  </Accordion>
</AccordionGroup>
