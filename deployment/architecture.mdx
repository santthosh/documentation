...
graph TD
    ...
    D -->|Promise.all — 5 agents| E[Amazon Bedrock<br/>us.anthropic.claude-sonnet-4-20250514-v1:0]
...
| ReviewAgent | Lambda (Node 20, ARM64) | 1024 MB, 300s timeout | Fetch diff, fan out to 5 agents, post results |
...
| Bedrock | Model inference | `us.anthropic.claude-sonnet-4-20250514-v1:0` | Powers all 5 review agents |
...
| 5x parallel Bedrock calls | 8–20 s |
...
graph TD
    ...
    C -->|Promise.all — 5 agents| D[LLM Provider<br/>Anthropic / Bedrock / Ollama / LiteLLM]
...
| Express server | Docker container | Port 3000 | Validate HMAC, fetch diff, run 5 agents, post results |
| Postgres | Database | Persistent storage | Installation config, review history |
| LLM provider | Any supported provider | Anthropic, Bedrock, Ollama, LiteLLM | Powers all 5 review agents |
...
| GitHub diff fetch | 200–800 ms |
| 5x parallel LLM calls | 5–30 s (varies by provider) |
...
Both architectures run the five review agents — **security**, **bugs**, **style**, **summary**, and **diagram** — in parallel via `Promise.all()`.
