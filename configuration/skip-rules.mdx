...
| All changed files are lock files | Skip | Remove lock patterns from `ignorePatterns` |
| All changed files in `dist/**` or `vendor/**` | Skip | Remove from `ignorePatterns` |
| PR title contains `[skip ci]` or `[skip review]` | Skip | Not configurable |
| All changed files are minified (*.min.js, *.min.css) | Skip | Remove minified patterns from `ignorePatterns` |
| All changed files are in generated directories (node_modules/**, build/**) | Skip | Remove from `ignorePatterns` |
| PR is pure documentation (only *.md files) | Skip | Remove *.md from `ignorePatterns` |
| PR author is MergeWatch (loop prevention) | Skip | Not configurable |
| PR has the `skip-review` label | Skip | Remove from `rules.ignoreLabels` |
...
Use `ignorePatterns` to skip PRs where all changed files match the specified glob patterns:
...
- If **all** changed files match `ignorePatterns`, the **entire PR is skipped**.
