...
<Step title="issue_comment.created (manual re-trigger)">
  A comment is posted on a pull request containing `@mergewatch review`. This works even when `auto_review` is set to `false` in your configuration.
</Step>
</Steps>
