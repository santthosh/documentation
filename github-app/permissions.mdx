...
| **Issues** | `read & write` | Create and update PR summary comments (GitHub's API uses the Issues endpoint for PR comments), list existing comments to avoid duplicates, and add reactions (e.g., :eyes: when a review starts). |
...
<Accordion title="Can MergeWatch modify my code?">
  No. MergeWatch has `contents: read` only. It cannot push commits, create branches, or modify any file. The only write operations are posting review comments (`pull_requests: read & write`), creating/updating PR summary comments (`issues: read & write`), and creating check runs (`checks: read & write`).
</Accordion>
