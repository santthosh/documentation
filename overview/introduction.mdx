...
| **Pricing** | No per-seat pricing — pay your Bedrock bill | $30/seat/month | $24/seat/month |
| **Review approach** | Multi-agent parallel pipeline | Full-codebase graph + multi-hop analysis | Single-pass diff review |
...
```
PR opened → GitHub webhook → WebhookHandler → ReviewAgent (async)
  ├── Security agent
  ├── Bug detection agent
  ├── Style agent
  ├── Summary agent
...
<Accordion title="How is this different from CodeRabbit?">
  ...
  **How you pay.** CodeRabbit charges $24/seat/month. MergeWatch has no per-seat fee. You pay your own Bedrock bill, which scales with usage, not team size.
