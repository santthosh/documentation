...
<Steps>
  ...
  <Step title="Async invocation">
    The handler invokes the **ReviewAgent Lambda** asynchronously using `lambda:InvokeFunction` with `InvocationType.Event` (fire-and-forget). The call returns immediately with a 202 status. AWS Lambda's built-in async invocation queue handles delivery, with up to 2 automatic retries on failure.
  </Step>
  <Step title="ReviewAgent invoked">
    The **ReviewAgent Lambda** (1024 MB, 300 s timeout) picks up the event from Lambda's internal async queue. The agent fetches the PR diff from the GitHub API using the installation token, then fans out to five specialized agents.
  </Step>
...
<CardGroup cols={2}>
  ...
  <Card title="ReviewAgent Lambda">
    ...
    - **Concurrency:** Triggered by async Lambda invoke (one invocation per event)
  </Card>
...
```mermaid
flowchart LR
    subgraph your["Your AWS Account"]
        direction LR
        GH["GitHub"] -->|webhook| AG["API Gateway"]
        AG --> WH["WebhookHandler"]
    ...
    style your fill:#0d1117,stroke:#00ff88,color:#fff
```
