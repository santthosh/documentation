...
sequenceDiagram
    ...
    participant MW as MergeWatch Lambda
    participant STS as sts:AssumeRole
    participant BR as Bedrock (Your Account)

    GH->>MW: Webhook (pull_request)
    MW->>GH: Fetch PR diff
    MW->>MW: Process diff in memory
    MW->>STS: Assume cross-account role
    STS-->>MW: Temporary credentials
    MW->>BR: Invoke model (your account, your bill)
    BR-->>MW: Model response
    MW->>GH: Post review comments
```
